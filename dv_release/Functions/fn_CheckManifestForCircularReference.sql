...
CREATE FUNCTION [dv_release].[fn_CheckManifestForCircularReference](@run_key int)
...
select @cr = case when isnull(r.run_manifest_key, 0) = 0 then 0 else 1 end
FROM wFindRoot r
WHERE r.run_manifest_key = r.run_manifest_prior_key 
  AND r.run_manifest_prior_key <> 0
  AND r.Distance > 0
select @cr = case when @cr is null then 0 else 1 end
return @cr
END