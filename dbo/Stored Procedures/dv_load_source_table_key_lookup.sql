...
BEGIN
    select @wrk_link_joins = 'LEFT JOIN ' + quotename(@c_hub_database) + '.' + quotename(coalesce(@c_hub_schema, @def_hub_schema, 'dbo')) + '.' + quotename((select [dbo].[fn_GetObjectName] (@c_hub_name, 'hub'))) + ' ' + @c_hub_abbreviation + @crlf + ' ON  '
    select  @wrk_link_keys +=  ' tmp.' + quotename([dbo].[fn_GetObjectName] ([dbo].[fn_GetObjectName] ([hub_name], 'hub'),'HubSurrogate')) + ' = link.' + quotename([dbo].[fn_GetObjectName] ([dbo].[fn_GetObjectName] ([hub_name], 'hub'),'HubSurrogate')) + @crlf + ' AND '
        ...
        select  @wrk_hub_joins += ', ' + @c_hub_abbreviation + '.' + quotename([dbo].[fn_GetObjectName] ([dbo].[fn_GetObjectName] ([hub_name], 'hub'),'HubSurrogate'))  + @crlf
        from(
